<script lang="ts">
  import { authApi } from '$lib/api/auth';
  import { authModalOpen } from '$lib/stores/authModal';
  import * as Dialog from '$lib/components/ui/dialog';
</script>

<Dialog.Root bind:open={$authModalOpen}>
  <Dialog.Content class="sm:max-w-md">
    <Dialog.Header>
      <Dialog.Title class="text-center">Sign In</Dialog.Title>
      <Dialog.Description class="text-center">
        Sign in to save your progress and compete on leaderboards.
      </Dialog.Description>
    </Dialog.Header>
    
    <div class="space-y-4 py-4">
      <a 
        href={authApi.getGoogleAuthUrl()}
        class="w-full px-6 py-3 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition flex items-center justify-center gap-3 text-gray-700 font-medium"
      >
        <svg class="w-5 h-5" viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        Sign in with Google
      </a>
      
      <button 
        disabled
        class="w-full px-6 py-3 bg-gray-100 border border-gray-200 rounded-lg cursor-not-allowed flex items-center justify-center gap-3 opacity-50"
        title="Coming soon"
      >
        <svg class="w-5 h-5" viewBox="0 0 23 23">
          <rect fill="#f35325" x="1" y="1" width="10" height="10"/>
          <rect fill="#81bc06" x="12" y="1" width="10" height="10"/>
          <rect fill="#05a6f0" x="1" y="12" width="10" height="10"/>
          <rect fill="#ffba08" x="12" y="12" width="10" height="10"/>
        </svg>
        Sign in with Microsoft
        <span class="text-xs text-gray-400">(Coming soon)</span>
      </button>
    </div>
  </Dialog.Content>
</Dialog.Root>
