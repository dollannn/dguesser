...
#[openapi(
    ...
    components(schemas(
        protocol::api::user::UserProfile,
        protocol::api::user::CurrentUser,
        protocol::api::user::UpdateProfileRequest,
        protocol::socket::payloads::JoinGamePayload,
        protocol::socket::payloads::SubmitGuessPayload,
    )),
    tags(
...
impl CurrentUserResponse {
    fn from_user(user: &db::users::User) -> Self {
        Self {
            id: user.id.clone(),  // Now String (usr_xxxxxxxxxxxx)
...
async fn create_game(
    ...
    // Game ID is generated by db layer using dguesser_core::game_id()
    let game = db::games::create(
        state.db(),
        mode,
        &auth.user_id,  // Now String (usr_xxx)
        &settings,
        join_code.as_deref(),
    )
    .await?;
...
async fn get_game(
    State(state): State<AppState>,
    auth: AuthUser,
    Path(id): Path<String>,  // Now String (gam_xxxxxxxxxxxx)
...
#[utoipa::path(
    ...
    params(
        ...
        ("round_number" = u8, Path, description = "Round number")
    ),
    request_body = SubmitGuessRequest,
    responses(
        (status = 200, description = "Guess result", body = GuessResult),
        (status = 400, description = "Invalid guess"),
        (status = 409, description = "Already guessed"),
    ),
    tag = "games"
)]
async fn submit_guess(
    State(state): State<AppState>,
    auth: AuthUser,
    Path((game_id, round_number)): Path<(String, u8)>,  // game_id now String
...
| GET | /docs | No | **Swagger UI (OpenAPI docs)** |
...
**Note:** All entity IDs use prefixed nanoid format (e.g., `usr_V1StGXR8_Z5j`, `gam_FybH2oF9Xaw8`)
